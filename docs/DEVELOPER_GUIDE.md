# Developer Guide: Sovereign Field LMS
...
```bash
# 1. Clone
git clone https://github.com/BlackFoxgamingstudio/field-lms.git
cd field-lms
...
```bash
export SBB_LMS_CONTENT_PATH=your_value_here
export SBB_CERT_ISSUER=your_value_here
export SBB_OFFLINE_CACHE_SIZE_MB=your_value_here
export SBB_PASS_THRESHOLD=your_value_here
...
│   ├── cli.py               # Typer CLI (sovereign-field-lms)
...
```bash
# Build
docker build -t sovereign-field-lms:dev .

# Run with env file
docker run --env-file .env -p 8789:8789 sovereign-field-lms:dev

# Full stack (with n8n network)
docker-compose up --build

# Logs
docker-compose logs -f sovereign-field-lms
...
3. Use **SovereignTools** node → configure endpoint to `http://localhost:8789`
...
| Issue | Diagnosis | Fix |
|-------|-----------|-----|
| Port `8789` in use | `lsof -i :8789` | Kill PID or change `SERVICE_PORT` |
